...
/** 一个可控的内存 EventAppender:可注入落盘失败、记录调用。 */
function makeMockStore(): EventAppender & {
  appended: Event[];
  failNext: number;
  appendCalls: number;
} {
  ...
  return {
    ...
    failNext: 0,
    appendCalls: 0,
    async appendEvent(_id: string, event: Event) {
      this.appendCalls++;
      if (this.failNext > 0) {
        this.failNext--;
...
describe("EventStream 持久化", () => {
  ...
  it("Public View 从历史与实时订阅剥离 providerReplay，内部投影仍可回放", async () => {
    ...
    stream.emitDelta({ kind: "thinking", phase: "start" });

    expect(stream.getEvents()[0]).toMatchObject({ providerReplay: replay });
    expect(JSON.stringify(stream.getPublicEvents())).not.toContain("ciphertext");
    expect(JSON.stringify(stream.getPublicEventsSince(0))).not.toContain(
      "providerReplay"
    );
    expect(JSON.stringify(publicSeen)).not.toContain("ciphertext");
    expect(publicSeen[1]).toEqual({
      delta: { kind: "thinking", phase: "start" },
    });
    expect(projectToMessages(stream.getEvents())[0]).toMatchObject({
      providerReplay: replay,
    });
  });

  it("compacted 只留在内部视图，生命周期事件继续公开", async () => {
    const stream = new EventStream();
    const publicSeen: unknown[] = [];
    stream.subscribePublic((item) => publicSeen.push(item));
    await stream.emit({
    ...
    expect(stream.getEvents().map((event) => event.type)).toContain("compacted");
    expect(stream.getPublicEvents().map((event) => event.type)).toEqual([
      "compaction_started",
      "compaction_completed",
    ]);
    expect((publicSeen as Array<{ type: string }>).map((event) => event.type)).toEqual([
      "compaction_started",
      "compaction_completed",
    ]);
  });
  ...
  it("上下文投影只应用最新 checkpoint，并保留其后的原始 tail", async () => {
    ...
    await stream.emit({
      ...
      model: "model",
      estimatedBeforeTokens: 100,
      estimatedAfterTokens: 50,
    });
    await stream.emit({ type: "user_message", source: "user", text: "new" });
    await stream.emit({
      type: "compacted",
      source: "agent",
      compactionId: "cmp-2",
      throughSeq: 4,
      replacesCompactionSeq: 3,
      summaryVersion: 1,
      summary: {
        objective: "second",
        confirmedDecisions: [],
      ...
      provider: "test",
      model: "model",
      estimatedBeforeTokens: 120,
      estimatedAfterTokens: 45,
    });
    await stream.emit({ type: "user_message", source: "user", text: "newest" });

    const projected = projectCompactedContext(stream.getEvents());
    expect(projected.messages).toEqual([{ role: "user", text: "newest" }]);
    expect(projected.systemContext[0]).toContain('"objective":"second"');
    expect(projected.checkpoint?.compactionId).toBe("cmp-2");
  });
});
