...
import type { LLMConfig } from "../server/options.js";
...
export function createLLMClient(cfg: LLMConfig): LLMClient {
  switch (cfg.provider) {
    ...
    case "openai": {
      ...
      return cfg.apiMode === "responses"
        ? new OpenAIResponsesClient(opts)
        : new OpenAIChatCompletionsClient(opts);
    }
    default:
...
function assertNever(value: never): never {
  throw new Error(`未知的 LLM provider：${String((value as { provider?: unknown }).provider)}`);
}
