...
const LABEL_INSTANCE = "tinyhands.instance";
...
export class DockerRuntime implements Runtime {
  ...
  private readonly instanceScope: string;
  ...
  constructor(opts: {
    ...
    this.instanceScope = opts.instanceScope;
    this.log = (opts.logger ?? noopLogger).child({ module: "docker-runtime" });
  }
  ...
  async create(): Promise<void> {
    ...
    const container = await this.docker.createContainer({
      ...
      Labels: {
        ...
        [LABEL_INSTANCE]: this.instanceScope,
      },
...
/**
 * 只清理 managed=true 且 instance label 匹配的残留容器。进程启动时清理上次遗留、
 * Host 显式 create/close 时执行。不抛异常:清理失败只记日志。
 */
export function deriveDockerInstanceScope(workspaceRoot: string): string {
...
export async function cleanupOrphanContainers(
  ...
  if (!instanceScope) throw new Error("Docker instanceScope 不能为空");
  ...
  try {
    ...
    log.info(
      { count: containers.length, instanceScope },
    ...
    for (const info of containers) {
      try {
        ...
        log.info(
          { containerId: info.Id.slice(0, 12), names: info.Names, instanceScope },
          "已清理残留容器"
        );
      } catch (err) {
        log.warn(
